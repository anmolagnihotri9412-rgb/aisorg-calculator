    ...
    speechService.startListening(
      async (text) => {
        ...
        if (expression) {
          ...
          speechService.speak("Sorry, I couldn't interpret that command.");
    ...
    <div className="flex flex-col h-screen max-w-md mx-auto bg-white dark:bg-gray-900 shadow-2xl relative overflow-hidden transition-colors">
      ...
      <div className="flex-1 flex flex-col justify-end px-6 py-8 text-right bg-gradient-to-b from-gray-50/50 to-white dark:from-gray-900 dark:to-gray-950 overflow-hidden">
        ...
        {result && (
            <div className="flex justify-end mt-4">
                <button onClick={copyResult} className="px-3 py-1 text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full hover:bg-indigo-100 dark:hover:bg-indigo-900/30 transition-colors">
                    Copy
                </button>
            </div>
        )}
      ...
      {isListening && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-indigo-600/95 backdrop-blur-xl text-white">
            ...
            <p className="text-sm opacity-70 mt-2">Speak your calculation clearly.</p>
            <button onClick={() => { setIsListening(false); speechService.stopListening(); }} className="mt-12 px-6 py-2 rounded-full border border-white/30 hover:bg-white/10 transition-colors">
                Cancel
            </button>
        </div>
      )}
      ...
      <div className="p-6 pb-10 bg-white dark:bg-gray-950 rounded-t-[3rem] shadow-[0_-8px_30px_rgb(0,0,0,0.04)] dark:shadow-none">
        ...
        <div className="flex justify-between mb-6 overflow-x-auto no-scrollbar gap-2 pb-2">
            {['sin', 'cos', 'tan', 'log', 'ln', 'sqrt', 'fact', '(', ')', 'π', 'e'].map(op => (
                <button key={op} onClick={() => handleInput(op === 'fact' ? 'fact(' : op + '(')} className="flex-shrink-0 px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg text-sm font-semibold btn-animation">
                    {op === 'fact' ? 'n!' : op}
                </button>
            ))}
        </div>
        ...
        <div className="calculator-grid">
          ...
          <button onClick={calculate} className="col-span-2 bg-indigo-600 hover:bg-indigo-700 text-white p-5 rounded-2xl font-bold text-2xl shadow-lg shadow-indigo-200 dark:shadow-none btn-animation">
            =
          </button>
        </div>

        <div className="flex justify-center mt-6">
            <button onClick={startVoiceInput} className="flex items-center gap-2 px-6 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-lg hover:shadow-xl hover:border-indigo-500 dark:hover:border-indigo-400 transition-all btn-animation">
